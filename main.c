...
int main() {
    ...
    // proses
    i = 0;
    while (matriks[i][0] != NIM && i != N) { i++;}

    int nilai_akhir = nilai(matriks[i][2], matriks[i][3], matriks[i][4]);
    int kelulusan = absen(matriks[i][1]);

    if      (90 <= nilai_akhir)                     { strcpy(indeks,"A");}
    else if (80 <= nilai_akhir && nilai_akhir < 90) { strcpy(indeks,"AB");}
    else if (75 <= nilai_akhir && nilai_akhir < 80) { strcpy(indeks,"B");}
    else if (65 <= nilai_akhir && nilai_akhir < 75) { strcpy(indeks,"BC");}
    else if (55 <= nilai_akhir && nilai_akhir < 65) { strcpy(indeks,"C");}
    else if (50 <= nilai_akhir && nilai_akhir < 55) { strcpy(indeks,"D");}
    else { strcpy(indeks,"E");}

    // output
    if (i == N)                               { printf("\nNIM tidak ada!"); }
    else if (kelulusan && nilai_akhir >= 55 ) { printf("\nNIM %d lulus dengan indeks %s", NIM, indeks); }
